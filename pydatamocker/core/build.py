...
from pydatamocker.types import FieldBuildRequest, FieldBuildResult,\
    FieldName, Field


def _apply(payload: FieldBuildRequest, build_results: Dict[FieldName, Series])\
        -> FieldBuildResult:
    size, field = payload
...
def build(requests: Collection[FieldBuildRequest]) \
        -> Collection[FieldBuildResult]:
    fields: List[Field] = [field for _, field in requests]
    requests_map = {
        field['name']: (size, field) for (size, field) in requests
    }
    levels = graph(fields)
    ...
    for level in levels:
        build_map = {
            **build_map,
            **dict((_apply(requests_map[name], build_map) for name in level))
        }
