...
def from_enum(values: List[Any], counts: Optional[List[Union[int, float]]] = None, shuffle: bool = True) -> FieldGenerator:
    weights: List[Union[int, float]] = counts or [1] * len(values)
    if shuffle:
        return lambda size: Series(values).sample(n=size, replace=True, weights=weights).reset_index(drop=True)
    else:
